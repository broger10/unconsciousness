...
import {
  ...
  Cross, Compass, Diamond, Sparkle, ChevronDown, Eye,
  type LucideIcon,
} from "lucide-react";
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6 pb-8">
        ...
        >
          ...
          <div className="flex items-center gap-3 text-xs text-text-muted font-ui">
            <span><Sun size={12} className="inline" /> {profile.sunSign}</span>
        ...
        >
          <div className="text-[10px] text-verdigris font-ui tracking-[0.2em] mb-3"><Sun size={10} className="inline" /> IL CIELO DI OGGI</div>
          {transits.length > 0 ? (
            ...
            <div className="space-y-2">
              {profile.natalChartData.criticalAspects.map((asp, i) => (
                <div key={i} className="text-sm text-text-secondary font-body flex items-start gap-2">
                  <Sparkle size={10} className="text-sienna shrink-0" />
                  <span>{asp}</span>
                </div>
              ))}
            </div>
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className="text-[10px] text-sienna font-ui tracking-[0.2em]"><Eye size={10} className="inline" /> MAPPA DELL&apos;OMBRA</div>
            <button
          ...
          <div className={`space-y-3 transition-all duration-500 ${showShadows ? "" : "curiosity-blur"}`}>
            {profile.shadows?.map((s, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-bg-glass">
                <Eye size={18} className="text-sienna shrink-0" />
                <div>
        ...
        {profile.mythologyNarrative && (
          ...
          >
            <div className="text-[10px] text-amber font-ui tracking-[0.2em] mb-4"><Diamond size={10} className="inline" /> LA TUA MITOLOGIA</div>
          ...
          <div className="glass rounded-2xl p-4 dimensional">
            ...
            <div className="space-y-2">
              {profile.values?.map((v, i) => (
                <div key={i} className="text-sm text-text-secondary font-body flex items-start gap-2">
                  <Sparkle size={10} className="text-verdigris shrink-0" /><span>{v}</span>
                </div>
              ))}
            </div>
          </div>
